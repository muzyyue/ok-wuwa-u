...
class BaseChar:
    ...
    def cycle(self):
        """循环执行 do_cycle（普攻/重击等），超时或 do_cycle 返回 False 时自动切人。"""
        self.cycle_start()
    ...
    def do_cycle(self):
        """子类重写：单步循环逻辑，返回 True 继续循环，False 跳出。"""
        return
    ...
    def sleep(self, sec, check_combat=True):
        """休眠指定秒数，check_combat=False 时跳过休眠中的战斗状态检测。"""
        if not check_combat:
    ...
    def available(self, box, check_color=True, check_cd=True):
        """判断技能按键是否可用：当前角色用 task 检测，非当前角色只检查 CD。"""

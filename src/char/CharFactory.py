...
_char_dict_raw = {  # 角色注册字典：Labels → {cls, char_type, ring_index, buff_time}
...
char_dict = {}  # 展开后的角色字典，处理多个 Labels 指向同一角色（如男/女漂泊者）
...
def get_char_by_pos(task, box, index, old_char):
    """通过模板匹配识别指定位置的角色，实例化对应的子类；未匹配则返回 BaseChar 兜底。"""

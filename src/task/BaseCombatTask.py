...
class NotInCombatException(Exception):
    """脱战异常：战斗中检测到脱战时抛出，打断当前角色执行。"""
    pass


class CharDeadException(NotInCombatException):
    """角色死亡异常：检测到复活弹窗时抛出（比 NotInCombatException 更严重）。"""
    pass


class CharRevivedException(CharDeadException):
    """角色已复活：中断当前战斗上下文，由 revive_action 处理后让任务重新进入。"""
    pass
...
class BaseCombatTask(CombatCheck):
    ...
    def refresh_cd(self):
        """OCR 扫描技能栏获取当前角色的共鸣/E/声骸冷却秒数，每帧仅执行一次。"""
    ...
    def revive_at_tower_and_heal(self):
        """死亡恢复：传送到周本入口作为稳定锚点，再传送到最近传送点回血。"""
        self.go_to_tower()
    ...
    def _oldest_switch_target(self, chars):
        """从角色列表中选出 last_switch_in_time 最小的（最早切上场）作为切换目标。"""
    ...
    def _switch_rule_3_target(self, candidates, allow_healer=True):
        """兜底切换规则：无 buff 奶妈 > 无 buff 副C > 最老主C > 其他。"""
    ...
    def _choose_switch_target(self, current_char, has_intro, target_low_con=False):
        """完整的切换目标选择主流程：优先级 → 入场/普通 → 切换CD过滤 → buff 分析。"""
        candidates = [
    ...
    def find_mouse_forte(self):
        """通过图像匹配检测鼠标重击回路（mouse_forte）图标是否亮起。"""
    ...
    def find_e_forte(self):
        """通过图像匹配检测 E 技能回路（e_forte）图标是否亮起。"""
    ...
    def set_key(self, key, box):
        """通过模板匹配自动识别技能按键（t/e/r/q），更新 key_config。"""
    ...
    def update_lib_portrait_icon(self):
        """通过头像旁的大招就绪图标（lib_ready）更新非当前角色的大招可用状态。"""
...
con_colors = [  # 六种元素协奏能量环的颜色范围：SPECTRO / ELECTRIC / FIRE / ICE / WIND / HAVOC
    {
        'r': (205, 235),
...
def convert_cd(text):
    """将 OCR 文本（如 '3.5s'）转换为浮点数冷却秒数，失败时返回 1。"""
    try:
